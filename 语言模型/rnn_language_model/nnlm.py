...
def train(corpus_path, save_weight=True):
    ...
    vocab = build_vocab(r"G:\NLP\Code\NLP_Code\4_语言模型\rnn_language_model\vocab.txt")       #建立字表
    ...
    if not save_weight:
        ...
        model_path = os.path.join(r"G:\NLP\Code\NLP_Code\4_语言模型\rnn_language_model\model", base_name)
        torch.save(model.state_dict(), model_path)
        return

#训练corpus文件夹下的所有语料，根据文件名将训练后的模型放到model文件夹
def train_all():
    for path in os.listdir(r"G:\NLP\Code\NLP_Code\4_语言模型\rnn_language_model\corpus"):
        corpus_path = os.path.join(r"G:\NLP\Code\NLP_Code\4_语言模型\rnn_language_model\corpus", path)
        train(corpus_path)


if __name__ == "__main__":
    # build_vocab_from_corpus("corpus/all.txt")
    train(r"G:\NLP\Code\NLP_Code\4_语言模型\rnn_language_model\corpus.txt", True)
    train_all()