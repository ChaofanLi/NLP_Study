...
def main():
    ...
    vocab_path = r"G:\NLP\Code\NLP_Code\8_序列标注任务\中文分词\chars.txt"  #字表文件路径
    corpus_path = r"G:\NLP\Code\NLP_Code\8_序列标注任务\中文分词\corpus"  #语料文件路径
    ...
    #保存模型
    torch.save(model.state_dict(), r"G:\NLP\Code\NLP_Code\8_序列标注任务\中文分词\model.pth")
    #保存词表
    writer = open(r"G:\NLP\Code\NLP_Code\8_序列标注任务\中文分词\vocab.json", "w", encoding="utf8")
...
if __name__ == "__main__":
    vocab_path = r"G:\NLP\Code\NLP_Code\8_序列标注任务\中文分词\chars.txt"
    vocab = build_vocab(vocab_path)
    main()
    print(jieba.lcut("今天天气不错我们去春游吧"))
    print(sequence_to_label("今天天气不错我们去春游吧"))
    print(sentence_to_sequence("今天天气不错我们去春游吧",vocab))
    input_strings = ["同时国内有望出台新汽车刺激方案",
                     "沪胶后市有望延续强势",
                     "经过两个交易日的强势调整后",
                     "昨日上海天然橡胶期货价格再度大幅上扬"]
    predict(r"G:\NLP\Code\NLP_Code\8_序列标注任务\中文分词\model.pth", r"G:\NLP\Code\NLP_Code\8_序列标注任务\中文分词\chars.txt", input_strings)
